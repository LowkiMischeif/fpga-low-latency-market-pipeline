...
imbalance, Q3.12 weights) are named constants in `rtl/market_pkg.sv`, and every
accumulation in the policy path saturates rather than wraps.
...
`risk_gate`'s `m_ready` is tied high, so every event in the integrated design
takes exactly `LATENCY_CYCLES`; `tb/tb_market_pipeline_top.sv` measures it.
...
```
 btnU --> reset_sync --> rst_n (every module)
...
| `rtl/replay_ctrl.sv` | On a debounced press, walks the ROM into the pipeline once, with no bubble or duplicate under backpressure. Ignores a press during a replay, during its lockout, or while a configuration is loading. |
| `rtl/cfg_loader.sv` | On reset release and whenever `sw[0]` changes, writes the selected preset (`rtl/mem/rom_cfg_baseline.mem` or `rtl/mem/rom_cfg_tuned.mem`) into `config_regs`, ending with the commit. |
...
Every module resets asynchronously on `negedge rst_n` (spec §3.3). An
...
nature and landing in a two-flop synchronizer; every output is read by a
person. Neither kind has a clock-edge relationship to constrain, so both are
